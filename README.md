...
- **Generic `indexed_shelves` settings field replaces `hive_shelf_id` + `user_journals_shelf_id` (issue #119).** The full-walk shelf list is now a single `Vec<i64>` on `GlobalSettings`. **Empty (the new default) means walk every shelf the index token can see** — so a fresh deployment gets useful semantic search out of the box without any `/settings` config. Non-empty restricts the walk to the listed shelves (e.g. `[42, 87]` carves out a scoped corpus). The DB schema migration auto-populates the new field with the union of any pre-existing `hive_shelf_id` + `user_journals_shelf_id` values on first boot, then drops both legacy columns (`DROP COLUMN IF EXISTS` on both SQLite 3.35+ and Postgres). The `/settings` UI now exposes a single comma-separated input. Fixes the v0.12.x footgun where non-BR deployments with neither legacy field set silently no-op'd the full walk and semantic search only saw webhook-pushed content.
